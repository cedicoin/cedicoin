# RapidCheck property-based testing for Cedicoin Core

## Concept

Property-based testing is experimentally being added to Cedicoin Core with
...
If RapidCheck is installed, Cedicoin Core will automatically run the
...
3. Configure Cedicoin Core with RapidCheck.

    `cd` to the directory of your local cedicoin repository and run
...
4. Build Cedicoin Core with RapidCheck.
    ...
    Making all in src
    [...]
    CXX      test/gen/test_cedicoin-crypto_gen.o
    CXX      test/test_cedicoin-key_properties.o
    ```
...
That's it! You are now running property-based tests in Cedicoin Core.
