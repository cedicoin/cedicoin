...
const char * const CEDICOIN_CONF_FILENAME = "cedicoin.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "cedicoind -foo=bar
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //cedicoin-tx using stdin
        std::string val;
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Cedicoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Cedicoin
    // Mac: ~/Library/Application Support/Cedicoin
    // Unix: ~/.cedicoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Cedicoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Cedicoin";
#else
    // Unix
    return pathRet / ".cedicoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", CEDICOIN_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Cedicoin Core copyright is not removed by accident
    if (copyright_devs.find("Cedicoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Cedicoin Core developers";
    }
